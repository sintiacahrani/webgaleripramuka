...
<x-navbar/>
		...
		<div id="title" class="flex items-center gap-[30px]">
			...
			<div class="flex items-center gap-3">
				<div class="w-[60px] h-[60px] flex shrink-0 rounded-full overflow-hidden">
					<img src="{{ Storage::url($author->first()->avatar) }}" alt="profile photo" />
				</div>
				<div class="flex flex-col">
					<p class="text-lg leading-[27px] font-semibold">{{ $author->first()->username }}</p>
					<span class="text-[#A3A6AE]">{{ $author->first()->occupation }}</span>
				</div>
			</div>
		</div>
		<div id="content-cards" class="grid grid-cols-3 gap-[30px]">

            @forelse ($author->first()->posts as $post)
	...
	<section id="Advertisement" class="max-w-[1130px] mx-auto flex justify-center mt-[70px]">
		<div class="flex flex-col gap-3 shrink-0 w-fit">
			<a href="{{ $banner_advertisments->link }}">
				<div class="w-[900px] h-[120px] flex shrink-0 border border-[#EEF0F7] rounded-2xl overflow-hidden">
					<img src="{{ Storage::url($banner_advertisments->thumbnail) }}" class="object-cover w-full h-full" alt="ads" />
				</div>
			</a>
